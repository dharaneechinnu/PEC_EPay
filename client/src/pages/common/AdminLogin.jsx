...
export default function AdminLogin() {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      const user = await authService.login('admin', { email, password });
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      // Decode JWT token (simple decode, no verification needed for demo)
      const payload = JSON.parse(atob(token.split('.')[1]));
      const user = await authService.login('admin', {
        email: payload.email,
        googleToken: token,
      });
  ...
  return (
    <Container>
      <Card>
        ...
        <Subtitle>Medical Emergency Fund Administration</Subtitle>
